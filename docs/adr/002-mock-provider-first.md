...
1. 直接接入真实 Provider (Qwen/OpenAI/StepFun)
...
- 已实现 Mock、Qwen、OpenAI、StepFun，业务层仍只依赖 `ExtractionProvider`。
- StepFun 已有一条合成图片运行到 Review 的真实链路证据；Mock 和接口契约测试不用于证明模型质量。
- 当前自动 Mock 降级只处理装配阶段的配置失败，并要求 Demo 双开关；运行时 API 失败走 FAILED、重试或人工创建。
- Playwright E2E 尚未隔离专用数据库和 Mock Provider，这是本 ADR“测试可控”尚未完全落实的部分。
