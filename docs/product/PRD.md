...
实现状态补充（2026-08-31）：文字流程已实现；图片／截图已支持单张上传、预览、保存及刷新恢复，Qwen/OpenAI 图片消息已接通。当前本地为 Mock，不能识别图片，真实模型效果待配置和验收。真实语音 ASR 仍不做，页面明确禁用；下文“语音转录”只指外部转写后的文字，不表示应用可以录音。微信来源仍由工作人员手动录入，未接入微信。验证范围见 [图片输入记录](../testing/image-intake.md)。
